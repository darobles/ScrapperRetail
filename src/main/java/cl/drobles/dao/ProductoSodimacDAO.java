 ...
 * @author drobles
 */
public class ProductoSodimacDAO extends AbstractProductoDAO{
    ...
    public void insertarProductos(List<Producto> productos) {
        ...
        try {
            ...
            try {
                stmt = conn.prepareStatement("INSERT INTO PRODUCTO_SODIMAC(sku,nombre,marca,categoria,precio,link,tienda,imagen, fecha_act) \n" +
                ...
                Logger.getLogger(ProductoSodimacDAO.class.getName()).log(Level.SEVERE, null, e);
            } finally {
                if (stmt != null) {
                    stmt.close();
                }
            ...
            try {
                ...
                Logger.getLogger(ProductoSodimacDAO.class.getName()).log(Level.SEVERE, null, ex);
            }
        } finally {
    ...
    public List<VistaProductoCategoria> obtProductosPorCategoria(String categoria) {
        ...
        try {
            String query = "SELECT p.sku,nombre,link,historico,fec_historico,ult_precio,fec_ult_precio FROM (SELECT sku,\n" +
                            ...
                            "       FROM PRODUCTO_SODIMAC\n" +
                            "       GROUP BY sku) h, (SELECT fecha_act AS fec_ult_precio,\n" +
                            "           precio AS ult_precio,\n" +
                            "            sku AS sku,\n" +
                            "           categoria AS categoria\n" +
                            "         FROM PRODUCTO_SODIMAC\n" +
                            "         WHERE id IN  (SELECT MAX(id) FROM PRODUCTO_SODIMAC WHERE categoria = ? GROUP BY sku) ) l, PRODUCTO_SODIMAC p\n" +
                            " WHERE l.sku = h.sku\n" +
                            " AND p.sku = l.sku\n" +
                            " AND p.categoria = ?\n" + 
                            " GROUP by sku"; 
