 ...
 * @author drobles
 */
public class ProductoEasyDAO extends AbstractProductoDAO{
    ...
    public void insertarProductos(List<Producto> productos) {
        ...
        try {
            ...
            try {
                stmt = conn.prepareStatement("INSERT INTO PRODUCTO_EASY(sku,nombre,marca,categoria,precio,link,tienda,imagen, fecha_act) \n"
                ...
                Logger.getLogger(ProductoEasyDAO.class.getName()).log(Level.SEVERE, null, e);
            } finally {
                if (stmt != null) {
                    stmt.close();
                }
            ...
            try {
                ...
                Logger.getLogger(ProductoEasyDAO.class.getName()).log(Level.SEVERE, null, ex);
            }
        } finally {
    ...
    public List<VistaProductoCategoria> obtProductosPorCategoria(String categoria) {
        ...
        try {
            String query = "SELECT p.sku,nombre,link,historico,fec_historico,ult_precio,fec_ult_precio FROM (SELECT sku,\n" +
                            ...
                            "         FROM PRODUCTO_EASY\n" +
                            "         WHERE id IN  (SELECT MAX(id) FROM PRODUCTO_EASY WHERE categoria = ? GROUP BY sku) ) l, PRODUCTO_EASY p\n" +
                            " WHERE l.sku = h.sku\n" +
                            " AND p.sku = l.sku\n" +
                            " AND p.categoria = ?\n" +
                            " GROUP BY sku"; 
